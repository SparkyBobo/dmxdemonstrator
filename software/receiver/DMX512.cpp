...
// - - - - -
// Modified by SparkBobo for use with the DMX Demonstrator.
// Documentation and samples are available at https://github.com/SparkyBobo/dmxdemonstrator/
// - - - - -
...
// Always use PORT1, DMX Demonstrator uses UART0 for serial.
...
void Dmx512Class::init(int mode)
{
...
void Dmx512Class::init(int mode, int dmxModePin)
{
  ...
  _dmxMaxChannel = DMX512_MAX; // The default in Receiver mode is reading all possible 512 channels.
  _dmxDataLastPtr = _dmxData + _dmxMaxChannel;

  // initialize the DMX buffer
  //  memset(_dmxData, 0, sizeof(_dmxData));
  for (int n = 0; n < DMX512_MAX + 1; n++)
    _dmxData[n] = 0;
...
void Dmx512Class::maxChannel(int channel)
{
  if (channel < 1)
    channel = 1;
  if (channel > DMX512_MAX)
    channel = DMX512_MAX;
...
uint8_t Dmx512Class::read(int channel)
{
  // adjust parameter
  if (channel < 1)
    channel = 1;
...
void Dmx512Class::write(int channel, uint8_t value)
{
  // adjust parameters
  if (channel < 1)
    channel = 1;
...
unsigned long Dmx512Class::noDataSince()
{
...
// return true when a packet has been received.
bool Dmx512Class::receive(uint8_t wait)
{
  bool ret = false;
